...
class OrderItems extends GenericTable {

	var $mTable = "j_order_items";
	...
	function getCountForMember($memberId, $search) {
		...
		if ($search != null)	
			$sql .= " and product_id in (select ID from j_orders where match(name) against ('" . $search . "'))";
		$this->mDb->query($sql);
		$this->mDb->bind(":member_id", $memberId);
		$rows = $this->mDb->single();
	...
	/**
	 * Get count of all rows for an order
	 * @param int $orderId
	 * @param string $search
	 * @return int $retVal count of rows
	 */
	function getCountForOrder($orderId, $search) {
		$retVal = 0;
		
		$sql ="SELECT count(*) as total from " . $this->mTable . " where purchase_id= :order_id";
		$this->mDb->query($sql);
		$this->mDb->bind(":order_id", $orderId);
		$rows = $this->mDb->single();
	...
	/**
	 * Get all rows for an order
	 * @param int $orderId 
 	 * @param int $row - starting row
	 * @param int rowsPerPage - rows to fetch
	 * @return array $rows 
	 */
	function getRowsForOrder($orderId, $startFrom, $rowsPerPage) {
		...
		$sql ="SELECT " . $this->mTable . ".*, j_products.name as productname, j_products.image as image from " . $this->mTable . " inner join j_products on j_products.ID = " . 
			$this->mTable . ".product_id where purchase_id = :order_id ";
		$sql .=  " limit " . $startFrom . ", " . $rowsPerPage;
		...
		$sql = " select * from " . $this->mTable . " where  purchase_id = :order_id and product_id=" .
	...
	/**
	 * Get all rows for an order grouped by meal type
	 * @param int $orderId 
 	 * @param int $row - starting row
	 * @param int rowsPerPage - rows to fetch
	 * @return array $rows 
	 */
	function getGroupedRowsForOrder($orderId, $startFrom, $rowsPerPage) {
