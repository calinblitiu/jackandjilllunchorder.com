...
class Subs extends GenericTable {
	...
	function getListForAUser($userId, $startFrom, $rowsPerPage, $sort) {
		$arrSort = ["id_desc"=>"ID desc", "id_asc"=>"id asc"
					];
		$sort = $arrSort[$sort];
	 ...
	 * @return array $row 
	 */
	function subsEntryExistsForMealType($userId, $studentId, $productId, $mealType) {
		
		$sql ="SELECT * from " . $this->mTable . " where user_id=:user_id and student_id=:student_id and product_id=:product_id and meal_type=:meal_type";
		$this->mDb->query($sql);
	 ...
	 * @return array $row 
	 */
	function subsEntryExists($userId, $studentId, $productId) {
		
		$sql ="SELECT * from " . $this->mTable . " where user_id=:user_id and student_id=:student_id and product_id=:product_id";
		$this->mDb->query($sql);
	...
	/**
	 * Check if a subs entry exists for userid and student id
	 * @param int $userId
	 * @param int $studentId
	 * @param date $orderDate
	 * @return array $row 
	 */
	function subsEntryExistsForUserAndStudent($userId, $studentId) {
		
		$sql ="SELECT * from " . $this->mTable . " where user_id=:user_id and student_id=:student_id";
		$this->mDb->query($sql);
