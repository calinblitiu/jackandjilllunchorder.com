<?php
	...
	if ($userId > 0) {
		...
		for($i = 0; $i < count($studentRows); $i++) {
			...
			if (
			     ($checkCart && $checkCart[0]["student_id"] == $studentRow["ID"]) ||
				 ($_SESSION["cart_student"] == $studentRow["ID"])
				) {
			   $sel = " selected ";


			   // if student is in session here then trigger getschool days function
			   if ($_SESSION["cart_student"] != null)
			   	 $getSchoolDays = 1;
			   $_SESSION["cart_student"] = null;
			} else
			   $sel = "";
	...
	// remove those categories for which there are no existing items for display as per current filter
	$catRows2 = array();
	foreach($catRows as $catRow) {
		$itemIds = $catRow["items"];
		$validCat = false;
...
<?php
	...
	if ($getSchoolDays)
		echo("<script> var getSchoolDaysFlag = 1; </script>");
	else
		echo("<script> var getSchoolDaysFlag  = 0; </script>");
