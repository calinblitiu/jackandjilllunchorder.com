...
function PutLink($URL,$txt)
{
	...
	// generate menu 
		...
		$pdf->SetXY($valueX, $currY);
		$pdf->SetFont($fontName,'',12);
		$pdf->Cell(20, 0, substr($schoolRow["name"],0,25), 0,0, "L", false);
		...
		foreach($catRows as $catRow) {
		  ...
		  foreach($irows as $irow) {
			  ...
			  $pdf->Cell(60, 12, $irow["name"], 0, 0, "L", true);
			  
			  $pdf->SetXY($priceX, $currY);
			  $pdf->Cell(30, 12,"$ " . number_format($irow["price"],2), 0, 0, "R", true);
		...
		foreach($catRows as $catRow) {
		  ...
		  foreach($irows as $irow) {
			  ...
			  //echo("y2=" . $currY . "<br>");
			  $pdf->SetXY($priceX, $currY);
			  $pdf->Cell(40, 12,"$ " . number_format($irow["price"],2), 0, 0, "R", true);
			  //$pdf->SetAutoPageBreak(true, $cutoffMargin);
