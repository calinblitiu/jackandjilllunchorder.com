<?php
	...
	foreach($arr as $id) {
		...
		// check if the status change is valid for this order
		$row = $orders->getRowById("ID", $id);
		if ($row["status"] == ORDER_STATUS_CANCELLED || $row["status"] == ORDER_STATUS_DELIVERED)
			continue;
		if ($row["status"] == ORDER_INPPROGRESS && $status == ORDER_RECEIVED)
			continue;
